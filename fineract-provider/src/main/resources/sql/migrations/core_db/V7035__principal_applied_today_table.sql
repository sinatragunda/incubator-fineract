...
SELECT ifnull(sum(m.principal_amount),0) as amount FROM m_loan m JOIN m_client mc ON m.client_id = mc.id WHERE m.submittedon_date = CURDATE() AND mc.office_id = ${officeId}','Used For keeping track of loans applied today', 0, 1);
