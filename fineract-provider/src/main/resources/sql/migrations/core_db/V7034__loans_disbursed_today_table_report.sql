...
INSERT INTO `stretchy_report` VALUES (null ,'LoansDisbursedToday', 'Table', '', 'Loan', 'SELECT ifnull(sum(m.amount),0) as amount FROM m_loan_transaction m WHERE m.transaction_date = CURDATE() AND m.transaction_type_enum = 1 AND m.office_id = ${officeId}','Used For keeping track of loans disbursed today', 0, 1);
